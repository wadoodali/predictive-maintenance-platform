...
with col1:
    ...
    air_temperature = st.number_input(
        "Air Temperature [K]", 250.0, 350.0, 298.1, 0.1
    )
    process_temperature = st.number_input(
        "Process Temperature [K]", 250.0, 350.0, 308.6, 0.1
    )

with col2:
    rotational_speed = st.number_input(
        "Rotational Speed [rpm]", 0.0, 5000.0, 1551.0, 1.0
    )
    torque = st.number_input(
        "Torque [Nm]", 0.0, 100.0, 42.8, 0.1
    )
    tool_wear = st.number_input(
        "Tool Wear [min]", 0.0, 300.0, 0.0, 1.0
    )
...
if st.button("Run Failure Prediction", type="primary", use_container_width=True):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            with col2:
                st.caption("Air Temp.")
                st.write(f"### {air_temperature:.1f} K")

            with col3:
                st.caption("Process Temp.")
                st.write(f"### {process_temperature:.1f} K")

            with col4:
                st.caption("Speed")
                st.write(f"### {rotational_speed:.0f} rpm")

            with col5:
                st.caption("Torque")
                st.write(f"### {torque:.1f} Nm")

            with col6:
                st.caption("Tool Wear")
                st.write(f"### {tool_wear:.0f} min")
            ...
            if feature_importance:
                ...
                for feature, importance in top_features:
                    ...
                    st.progress(min(100, int(percentage)))

                if other_features:
                    with st.expander("Other model features"):
                ...
                if len(top_features) >= 2:
                    st.info(
                        f"**Model insight:** {top_features[0][0]} and "
                        f"{top_features[1][0]} are the strongest model "
                        f"drivers for this prediction."
                    )
